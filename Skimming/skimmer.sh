...
FILTERS_OUT=( "Tau_Run" "TauES_DM0up" "DY" "ST" "TT" "TauES_DM0down" "WZ" "WW")
FILTER_OUT=""
FILTERS_IN=()
...
PATH_TO_SKIMS="/hdfs/store/user/jnatoli/NanoTauAnalysis/analysis/Wto3pi_2018_fromstep1_skimmed/"
...
for SAMPLE_TYPE in $(ls ${PATH_TO_NTUPLES}); do
	...
	for filter_in in "${FILTERS_IN[@]}"; do
		...
		if [[ ! "${ROOT_FILE}" == *"${filter_in}"* ]]; then
		    echo "Include '${filter_in}' did not match ${ROOT_FILE}, SKIPPING"
		    SKIP=1
		    break
		fi
	    fi  
	...
	# Make sure this compiles ok...
	if $(g++ $RCOMP skimOver.C -o ${SKIMOVER_OUT}); then
	...
	# and move...
	echo ">> gfal-copy --force small.root ${PATH_TO_SKIMS}/${SAMPLE_TYPE}/${ROOT_FILE}"
	gfal-copy --force small.root ${PATH_TO_SKIMS}/${SAMPLE_TYPE}/${ROOT_FILE}
...
# for j in Data QCD ST TTV VV VVV W+Jets; do for i in $(ls /Volumes/WineCellar/KState/analysis/Wto3pi_2018/${j}); do ./skimquick /Volumes/WineCellar/KState/analysis/Wto3pi_2018/${j}/${i}; g++ $RCOMP skimOver.C -o overskim; ./overskim; mv small.root /Volumes/WineCellar/KState/skims/Wto3pi_2018/${j}/${i}; done
