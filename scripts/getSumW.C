...
void getSumW(std::string infname, std::string prefix, std::string njets) {
  ...
  if (file.is_open()) {
	...
	if ( fname.find("RunII") != std::string::npos) {
	  nano = "NanoAODv9";
	  extend = 7;
	} else {
	  nano = "Nanoaodv12";
	...
	std::string mini;
	// if ( fname.find("NanoAODv12-999999") != std::string::npos)
	if ( fname.find(nano + "-999999") != std::string::npos)
	  mini = fname.substr( fname.find( "999999_") + extend);
	else
	  // mini = fname.substr( fname.find( "NanoAODv12_") + 10);
	  mini = fname.substr( fname.find( nano + "_") + extend + 3);
